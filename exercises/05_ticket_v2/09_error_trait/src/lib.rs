...
impl Display for TicketNewError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::TitleError(err) => write!(f, "{err}"),
            Self::DescriptionError(err) => write!(f, "{err}"),
        }
    }
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Err(TicketNewError::DescriptionError(_err)) => easy_ticket(title, "Description not provided".to_string(), status)
    }
}
